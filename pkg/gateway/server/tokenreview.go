...
func NewGatewayTokenReviewer(gatewayClient *client.Client, gptClient *gptscript.GPTScript, dispatcher *dispatcher.Dispatcher, tokenService *persistent.TokenService) authenticator.Request {
...
func (g *gatewayTokenReview) AuthenticateRequest(req *http.Request) (*authenticator.Response, bool, error) {
	...
	if g.tokenService != nil {
		if tokenCtx, err := g.tokenService.DecodeToken(req.Context(), bearer); err == nil {
			...
			// GPTScript provider, so we ignore errors here.
			_ = populateContext(req, g.gptClient, g.dispatcher, namespace, name)
	...
	// may not exist as a running GPTScript provider, so we ignore errors here.
