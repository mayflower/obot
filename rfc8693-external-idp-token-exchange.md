...
|---|---|
| `urn:ietf:params:oauth:token-type:id_token` | External IdP ID token to Obot MCP access token |
...
| RFC 8693 external IdP exchange | Programmatic MCP Gateway access by an external app that already has an IdP ID token | Multiple validators can be active in parallel |
...
```text
Dex ID token      -> POST /oauth/token -> generic OIDC validator -> Obot MCP access token
Google ID token   -> POST /oauth/token -> Google validator       -> Obot MCP access token
Entra ID token    -> POST /oauth/token -> Entra validator        -> Obot MCP access token
```
...
7. Obot authenticates the access token, deploys or locates the MCP server, replaces the external bearer token with an internal MCP proxy JWT, and proxies the request.
8. The MCP server returns tool/resource results to the external service.
```
